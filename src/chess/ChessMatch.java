...
public class ChessMatch {
	...
	private Board board;
	private boolean check; //padrão começa como false
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) { //testando se para a peça de origem a posição de destino não é um movimento possível
	...
	private boolean testCheck(Color color) {
		...
		for (Piece p: opponentPieces) { //percorre todas as peças do oponente 
			boolean [][] mat = p.possibleMoves();
			if (mat[kingPosition.getRow()][kingPosition.getColumn()]) { // se nessa posição alguma peça do oponente oferece o check retorna true
				return true;
			}
		}
		return false;
